      subroutine interp2d_2dfields(nxi,nyi,lonin,latin,fin, &
      ...
      integer :: nxo,nyo,nxi,nyi,i,j,ii,jj
      integer :: sx,ex,sy,ey
      integer :: res(2),a(2),b(2),c(2),d(2)
      real :: lonin(nxi,nyi),latin(nxi,nyi),fin(nxi,nyi)
      real :: lonout(nxo,nyo),latout(nxo,nyo),fout(nxo,nyo)
...
!$OMP  COLLAPSE(2)     &
!$OMP DEFAULT (NONE)   &
!$OMP SHARED(fout,fin,latin,latout,lonin,lonout,nxo,nyo,nxi,nyi,sy,sx,ey,ex)  &
...
!$OMP  COLLAPSE(2)     &
!$OMP DEFAULT (NONE)   &
!$OMP SHARED(fout,fin,sy,sx,ey,ex,WA,WB,WC,WD,XA,XB,XC,XD,YA,YB,YC,YD,flag_interp) &             
!$OMP PRIVATE(i,j)
      do j=sy,ey
      do i=sx,ex
        fout(i,j)=fin(XA(i,j),YA(i,j))*WA(i,j)+   &
                  fin(XB(i,j),YB(i,j))*WB(i,j)+   &
                  fin(XC(i,j),YC(i,j))*WC(i,j)+   &
                  fin(XD(i,j),YD(i,j))*WD(i,j)
      enddo
      enddo
!$OMP END PARALLEL DO
!$OMP BARRIER
